import sys
import csv
from generators.orderfile.order_generator import OrderGenerator

class DisplayData(OrderGenerator):
    def print_console(self, count):
        data = self.generate_orders(count)
        for user_id, orderAt, store_id, order_id in data:
            print(f"Id: {user_id}\nOrderAt: {orderAt}\nStoreId: {store_id}\nUserId: {order_id}\n")
    ...
    def print_csv(self, count):
        ...
        with open('orders.csv', 'w', newline='', encoding='utf-8') as file:
            ...
            # CSV 헤더 작성
            writer.writerow(['user_id', 'orderAt', 'store_id', 'order_id'])
            # 각 행에 사용자 데이터 작성
            for user_id, orderAt, store_id, order_id in data:
                writer.writerow([user_id, orderAt, store_id, order_id])
        print("CSV 파일에 저장 완료")
